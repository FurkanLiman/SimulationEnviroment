import environment.startUpWindow as startUpWindow
import vpython as vp
import time
import environment.environment as environment
import creature.creature as creature
import random
import copy
from ete3 import Tree, TreeStyle, NodeStyle, TextFace
from PIL import Image
import drawCharts.radarChart as radarChart
import drawCharts.lineChart as lineChart
import os
import naturalDisasters as naturalDisasters
import math
import creature.mutationFactors as mutationFactors
import aiModeling.modelTrainingDataCollector as modelTrainingDataCollector

def main(startingParam):
    developerMode = startingParam[0]
    if developerMode:
        startUpWindow.updateConfig(startingParam[1],startingParam[2],startingParam[3],startingParam[4],startingParam[5])
    else:
    ...
    class Chars:
        ...
        def results(self):
            ...
            for char in self.chars.values():
                if char.hunger:
                    ...
                    if mutationState and (char.gene not in genePool)and (oldGene in genePool):
                        nstyle = NodeStyle()
                        nstyle["size"] = 10
                        nstyle["fgcolor"] = ("#{:02x}{:02x}{:02x}".format(int(char.body.color.x*255), int(char.body.color.y*255), int(char.body.color.z*255)))
        ...
        def dailyStats(self, winners,lenLosers):
            ...
            if len(winners):
                ...
                modelTrainingDataCollector.collectData(day= day,
                                        lenWinners=len(winners),
                                        lenLosers=lenLosers,
                                        isDisasterInDay= isDisasterInDay,
                                        meanSpeed=sumS["speed"]/len(winners),
                                        meanVision=sumS["vision"]/len(winners),
                                        meanVisionRadius=sumS["visionRadius"]/len(winners),
                                        meanImmunity=sumS["immunity"]/len(winners),
                                        meanDurability=sumS["durability"]/len(winners),
                                        mutationFactors=mutationFactors.mutationProbability)       
        ...
        def resetPos(self):
            ...
            for j in range(4):
                if say%4 != 0:
                    ...
                    don = number//4
                for i in range(don):
                    char = self.chars[keys[idm]]
                    pos = round(positions[j][0] * 0.85 / (number / 4), 2) * i - round(positions[j][0] * 0.85 / 2, 2)
                    actual_pos = (pos, 0, positions[j][2]) if j % 2 == 0 else (positions[j][2], 0, pos)
                    axis = vp.vector(1,0,0)
                    char.body.axis = axis
                    char.body.pos=vp.vector(actual_pos[0],actual_pos[1],actual_pos[2])            
                    char.idText.pos = char.body.pos
                    char.angle.pos.x = (char.genomes["visionRadius"]/2)*axis.x + char.body.pos.x
                    char.angle.pos.z = (char.genomes["visionRadius"]/2)*axis.z + char.body.pos.z
                    char.angle.axis = char.body.axis
    ...
    while True:
        if env.running:
            ...
            if times >= dayLength:
                ...
                times = 0
                day +=1
                if day >= startUpConfigurations["dayLimit"] or len(dozenChar.chars.keys()) == 0:
                    break
            times += 1
    ...
    if not developerMode:
        ...
        modelTrainingDataCollector.writeFile()
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        variable = ast.literal_eval(sys.argv[1])
    else:
        variable = [False]
    main(variable)